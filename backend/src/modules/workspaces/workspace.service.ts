...
import { computeEffectiveRole } from '../../permissions/effective-role';
...
export class WorkspaceService {
  ...
  async createWorkspace(userId: string, data: CreateWorkspaceDto) {
    ...
    if (memberships.length > 0) {
      const canCreateWorkspace = memberships.some((membership) => {
        const effectiveRole = computeEffectiveRole(membership.role, userId, membership.workspace.ownerId);
  ...
  async addMember(workspaceId: string, userId: string, targetEmail: string, role: Role) {
    const access = await resolveWorkspaceAccess(workspaceId, userId);
